...
$repositoryJson = ConvertTo-Json -InputObject $requestedRepository -Compress
$protectedActionsJson = ConvertTo-Json `
    -InputObject ([object[]]$protectedActions.ToArray()) `
    -Compress
...
) ('codex-dispatch-worker-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    $processResult = Invoke-WorkerProcess `
        ...
        -WorkingDirectory $authorizedPath `
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$processResult.TransportError)) {
        ...
        return New-WorkerOutput `
            -Status 'failed' -Project $publicProject -ExitCode ([int]$processResult.ExitCode) `
            -Diagnostic $diagnostic
    }
    ...
    try {
        $threadId = Get-WorkerThreadId -EventStream ([string]$processResult.StandardOutput)
    }
    catch {
        $diagnostic = ConvertTo-WorkerSafeDiagnostic `
        ...
        return New-WorkerOutput `
            -Status 'failed' -Project $publicProject -ExitCode 0 -Diagnostic $diagnostic
    }

    if (-not (Test-Path -LiteralPath $finalResultPath -PathType Leaf)) {
        return New-WorkerOutput `
            -Status 'failed' -Project $publicProject -ThreadId $threadId -ExitCode 0 `
